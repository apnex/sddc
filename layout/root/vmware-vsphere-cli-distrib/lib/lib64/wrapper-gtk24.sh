...
libvmwareui="$libs/libvmwareui.so/libvmwareui.so"
...
vm_fix_gtk() {
   ...
   # libglade
   # I don't even think this is used anymore, but while we still have the
   # linkage, we consider it part of the "base" gtk set because while it's
   # symbol compatible, it's not actually binary compatible enough to avoid
...
vm_run() {
   ...
   while true; do
      ...
      lddOutput="`LANGUAGE=C LANG=C ldd "$binary" 2>/dev/null`"
   ...
   if [ "$VMWARE_USE_SHIPPED_GTK" = '' ]; then
      ...
      if echo "$lddUiLibOutput" | grep -Eq 'GLIBCXX_3.4|CXXABI_1.3'; then
   ...
   if [ "$VMWARE_USE_SHIPPED_GTK" = '' ]; then
      if echo "$lddOutput" | grep -Eq 'GLIBCXX_3.4|CXXABI_1.3'; then
         # No sufficiently new C++ runtime environment on the system.
         vm_append_to_library_path "`vm_fix_cplusplus`"
      fi
   fi
