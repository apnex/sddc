...
sub connect {
   ...
   if (Opts::option_is_set('sessionfile')) {
      ...
      my $pyexe = __FILE__;
      $pyexe =~ s,VICommon.pm$,pyexe/ssoclient,;
      # For windows the ssoclient available on the same dir
      if (!-e $pyexe) {
         $pyexe = __FILE__;
         $pyexe =~ s,VICommon.pm$,ssoclient,;
      }
      ...
      if (defined ($url)) {
         ...
         $pyCmd =  "\"$pyexe\"" . " --server=$vcUrl --user=$username --password=$password --stsurl=$ssourl ";
      } else {
         $pyCmd =  "\"$pyexe\"" . " --user=$username --password=$password --stsurl=$ssourl ";
      }
      #if (Opts::option_is_set('savesamltoken')) {
      #   $pyCmd .= " --savesamltoken " . Opts::get_option('savesamltoken');
      #}
      $pyCmd .= " |";
      my $session_id = undef;
      my $vcUrl = undef;
      eval {
         ...
         my $item = undef;
         foreach $item (<CMD>) {
           if ($item =~ /.*vmware_soap_session=.*/) {
              $session_id = $item;
              $session_id =~ s/.*vmware_soap_session=\"(.*)\".*\n/$1/;
           }
           if ($item =~ /^Server Url is ::.*/) {
              $vcUrl = $item;
              $vcUrl =~ s/.*Server Url is :: (.*)\n/$1/;
           }
         }
         close CMD;
      };
